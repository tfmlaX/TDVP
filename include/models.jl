...
function correlatedenvironmentmpo(N::Int, Nm::Int, dhilbert::Int; E=[], J=0.2, chainparams, s=1, α=0.02, ωc=1, R=1, c_phonon=1, beta ="inf", κ=1, signature = [], issoft=false, compression=false)
    #=
    ...
    if length(signature)<N
        ...
        vcat(signature, ones(N-length(signature)))
    end
    ...
    for x = 1:N-1
        ...
        i = 2 # index counter
        M[1,i,:,:] = x==1 ? 0*c : J*c # The first site (the switch) is not coupled to the other sites
        M[1,i+1,:,:] = x==1 ? 0*cd : J*cd
        M[1,i+2*x,:,:] = M[1,i+1+2*x,:,:] = signature[x]*P
    ...
    M[1,1,:,:] = M[D,D,:,:] = u
    i = 2 # index counter
    M[1,D-2,:,:] = M[1,D-1,:,:] = signature[N]*P # The top of the barrier is connected to the bath. Comment to disconnect from the bath
    M[1,D,:,:] = E[N]*P
	...
	for m = 1:Nm-1
            ...
            while i<D
                ...
                #couplingcoeff = a==1 ? sqrt(κ)*γ(1, m, issoft,beta=beta) : γ(a, m, issoft,beta=beta)
